...
def update_chart(chart_json: json, params: list, annotation: json, filename: str = 'chart') -> np.ndarray:
    ...
    chart_json['vconcat'][0]['layer'][0]['encoding']['color']['condition']['test'] = f"datum.Entity === '{annotation['tasks'][0]['entity']}'"
...
def get_bbox(svg_file, annotation:json) -> np.ndarray:
    ...
    for element in PNT:
        if element.getAttribute('aria-label') == annotation['tasks'][0]['aria-label']:
